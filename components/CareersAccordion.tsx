              ...
              return (
                <div
                  ...
                  className={cx(
                    ...
                    !hasLangTitle && "is-no-lang" // ★ここで追加
                  )}
                >
                  <div className={cx(
                    "pre:w-[142px] pre:sm:w-full pre:sm:sp-mb-[60] pre:shrink-0",
                    !hasLangTitle && "is-no-lang pre:sm:sp-mt-[-60]" // ★ここで追加
                  )}>
                    <h4
                  ...
                  {/* 見出しが無い時は全幅に */}
                  <div className={cx(hasLangTitle ? "pre:w-[calc(100%-142px-15px)] pre:sm:w-full" : "pre:w-full")}>
                    {/* 以下そのまま */}
                    {sec.kind === "text" && (
                      <>
                        ...
                        {sec.items.map((it, i) => (
                          <li key={i} className="pre:sm:sp-pl-[72] pre:sm:relative">
                            ...
                            <div className="pre:w-[calc(319/496*100%)] pre:sm:sp-w-[177]">
                              <p className="pre:text-[12px] pre:leading-[180%] pre:font-dnp pre:font-light pre:sm:sp-fs-[12]">{it.desc ?? ""}</p>
                            </div>
                          </li>
                        ))}
                      </ul>
