...
type Section =
  | {
      kind: "text";
      en?: string; // ★ optionalに（空/未定義を許容）
      ja?: string; // ★ optionalに（空/未定義を許容）
      text: string;
      textNote?: string;
    }
  | {
    ...
    <div className="accordion careers-accordion group" id={id}>
      ...
      <div className="accordion__inner careers-accordion__inner pre:grid-cols-[calc(436/1401*100%)_1fr] pre:gap-x-[calc(158/1401*100%)] pre:items-start pre:sm:!block">
        <div className="pre:sticky pre:top-[140px] careers-accordion-illust pre:pl-[calc(114/436*100%)] pre:pt-[70px] pre:sm:relative pre:sm:top-auto pre:sm:pt-0 pre:sm:p-0 pre:sm:sp-mt-[50]">
          <div
            className={cx(
              "pre:w-full pre:sm:sp-pl-[89] pre:sm:flex pre:sm:justify-center",
              illustWrapClassName
            )}
          >
            <Image
        ...
        <div className="accordion__inner-content careers-accordion__inner-content">
          {sections.map((sec, idx) => {
            // ★ en/ja が両方ない（空/未定義）なら false
            const hasLangTitle = Boolean(sec.en?.trim() || sec.ja?.trim());

            return (
              <div
                className={cx(
                  "careers-accordion-box",
                  // ★ en/ja がある時だけ区切り（padding-top + border-top）を付ける
                  hasLangTitle && "pre:pt-[60px] pre:border-t pre:border-darkGray"
                )}
                key={`${sec.en ?? "sec"}-${idx}`}
              >
                {/* ★ en/ja がある時だけ見出しブロックを描画 */}
                {hasLangTitle && (
                ...
                {/* ★ 見出しが無い時は横幅計算をやめて全幅にする */}
                <div
                  className={cx(
                    hasLangTitle
                      ? "pre:w-[calc(100%-142px-15px)] pre:sm:w-full"
                      : "pre:w-full"
                  )}
                >
                  {sec.kind === "text" && (
                    <>
                      <p className="pre:text-[14px] pre:leading-[180%] pre:sm:sp-fs-[14]">
                        {sec.text}
                      </p>
                      {sec.textNote && (
                        <p className="pre:text-[12px] pre:sm:sp-fs-[12]">
                          {sec.textNote}
                        </p>
                      )}
                    </>
                  ...
                  {sec.kind === "flow" && (
                    <ul className="pre:[&>li]:text-[12px] pre:[&>li]:pb-3 pre:[&>li]:sm:sp-pb-[25] pre:[&>li]:mb-3 pre:[&>li]:sm:sp-mb-[25] pre:[&>li:last-child]:mb-0 pre:[&>li:last-child]:border-b-0 pre:[&>li]:border-b pre:[&>li]:border-darkGray pre:[&>li]:border-solid pre:[&>li]:flex pre:[&>li]:items-center pre:[&>li]:sm:flex-wrap pre:[&>li]:sm:justify-between">
                      {sec.items.map((it, i) => (
                        <li
                          key={i}
                          className="pre:sm:sp-pl-[72] pre:sm:relative"
                        >
                          <div className="pre:w-[calc(71/496*100%)] pre:sm:absolute pre:sm:top-0 pre:sm:left-0 pre:sm:sp-w-[72]">
                            <p className="pre:text-[20px] pre:font-gt pre:font-light pre:sm:sp-fs-[20] pre:leading-none">
                              {it.no}
                            </p>
                          </div>
                          <div className="pre:w-[calc(106/496*100%)] pre:sm:sp-w-[177] pre:sm:sp-mb-[20]">
                            <p className="pre:text-[14px] pre:font-dnp pre:font-light pre:sm:sp-fs-[14] pre:leading-none">
                              {it.title}
                            </p>
                          </div>
